...
public class CurrentExpeditionFragment extends Fragment {

    public static final String TAG = "CurrentExpedition";
    // Variables.
    ...
    DatabaseReference databaseRef, databaseImagesRef, videoRef;
    ...
    private Button btnUploadVideo;//button to upload
    private VideoView videoView;//videoPlayer

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ViewGroup root = (ViewGroup) inflater.inflate(R.layout.current_expedition_fragment_view, container, false);

        startLocation = root.findViewById(R.id.startLocation);
        endLocation = root.findViewById(R.id.endLocation);
        openMaps = root.findViewById(R.id.mapsButton);
        saveButton = root.findViewById(R.id.saveButton);
        uploadButton = root.findViewById(R.id.uploadButton);
        imagesView = root.findViewById(R.id.imagesView);

        imagesView.setHasFixedSize(true);
        imagesView.setLayoutManager(new LinearLayoutManager( getActivity()));
        ...
        videoRef = FirebaseDatabase.getInstance().getReference().child("Current Expedition videos");
        ...
        databaseImagesRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists() && upload == false)
                {
                    ...
                    customAdapter = new CustomAdapter(getActivity(), imageClassList);
        ...
        videoRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists() && upload == false)
                {
                    ...
                    for(DataSnapshot postSnapshot : dataSnapshot.getChildren())
                    {
                        vs = postSnapshot.getValue(VideoClass.class);

                    }
                    videoView.setVideoPath(vs.getVideoUri());
                }
            }
        ...
        btnUploadVideo = root.findViewById(R.id.uploadVideo);
        videoView = root.findViewById(R.id.videoView);
        // set upload video click listener
        btnUploadVideo.setOnClickListener(view -> videoChooser());
        ...
        // videoView the touch event to make the setOnClickListener invalid,
        // so you need to use setOnTouchListener
    ...
    /**
     * upload video file to server and update video preview uri
     * @param videoName video name
     * @param videoUri local video uri
     */
    private void upLoadVideo(String videoName, Uri videoUri) {
        StorageReference ref = storageRef.child(videoName);
        ref.putFile(videoUri).addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
            @Override
            public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                Task<Uri> uriTask = taskSnapshot.getStorage().getDownloadUrl();
                Uri downloadUri = uriTask.getResult();
                // update local videoView uri
                videoView.setVideoURI(downloadUri);
                Toast.makeText(getContext(), "upload success", Toast.LENGTH_SHORT).show();
                saveVideoRef(videoName,videoUri.toString());
            }
        }).addOnFailureListener(e -> {
            Log.i(TAG, "putFile: failed");
            Toast.makeText(getContext(), "upload failed", Toast.LENGTH_SHORT).show();});
    }

    /**
     * save video upload uri
     * @param videoName video name
     * @param videoUri video uri which in server.
     */
    private void saveVideoRef(String videoName, String videoUri) {
        String id = videoRef.push().getKey();

        Map<String, Object> videoMap = new HashMap<>();
        videoMap.put("videoUri", videoUri);
        videoMap.put("videoName", videoName);

        videoRef.child(id).setValue(videoMap).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                Log.i(TAG, "onComplete: saveVideoRef");
            }
        });
    }
